import { getDb } from '../../apps/server/src/db/index';
import { runSeed } from '../../apps/server/src/db/seed';
import { calculateTrialState } from '../src/utils/trial';
import { STOCK_MOVEMENT_CODES } from '../src/constants';
import { formatDZD } from '../src/utils/currency';
import { formatProductCode, generateBarcodeValue } from '../src/utils/barcode';
...
async function runEndToEndVerification() {
  ...
  // 2. Vérification des Magasins, Utilisateurs et Couleurs
  ...
  console.log(`   - Couleurs pré-remplies : ${colors.length} (Objectif ~120 couleurs)`);
  ...
  if (lastMov.movement_code === 90) {
    ...
    console.log(`   - Ancien stock (à afficher en rouge barré) : ${lastMov.qty_before}`);
    console.log(`   - Nouveau stock (à afficher en vert) : ${lastMov.qty_after}`);
