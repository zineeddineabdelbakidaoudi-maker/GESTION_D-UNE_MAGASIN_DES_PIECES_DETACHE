...
router.post('/', authenticateToken, requirePermission('users', 'edit'), async (req: AuthRequest, res: Response) => {
  const { username, password, fullName, role, storeId, permissions } = req.body || {};
  if (!username || !password || !fullName) {
    return res.status(400).json({ error: 'Nom complet, identifiant et mot de passe requis' });
  }
  ...
  try {
    ...
    const insertUser = rawDb.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 1)
    `);
    const userRes = insertUser.run(username.trim(), passwordHash, fullName.trim(), role || 'cashier', storeId ? Number(storeId) : null);
    ...
    for (const mod of SYSTEM_MODULES) {
      const p = permissions?.find((item: any) => item.module === mod);
    ...
    res.status(201).json({ id: userId, username, fullName, role: role || 'cashier', storeId });
  } catch (err: any) {
...
router.put('/:id/permissions', authenticateToken, requirePermission('users', 'edit'), (req: AuthRequest, res: Response) => {
  ...
  const permissionsList = Array.isArray(req.body?.permissions) ? req.body.permissions : (Array.isArray(req.body) ? req.body : []);
  const { rawDb } = getDb();
